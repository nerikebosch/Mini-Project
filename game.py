...
while True:
    ...
    if game_mode is None:
        ...
        vsAIButton = pygame.Rect(1.5 * (width / 8), (height / 2), (width / 4), 80)  # rectangular object
        ...
        play2Button = pygame.Rect(4.5 * (width / 8), (height / 2), (width / 4), 80)
        ...
        if click == 1:
            ...
            if vsAIButton.collidepoint(mouse):
                time.sleep(0.2)
                game_mode = "AI"
            elif play2Button.collidepoint(mouse):  # test if the clicked point is inside the rectangle
                time.sleep(0.2)
                game_mode = "2P"
                user = ttt.X
    ...
    # AI mode: Let user choose a player.
        ...
        # Draw buttons
        playXButton = pygame.Rect((width / 8), (height / 2), width / 4, 80)
        ...
        playOButton = pygame.Rect(5 * (width / 8), (height / 2), width / 4, 80)
    ...
    else:  # (game_mode = 2P) or (AI and user)
            ...
            for i in range(3):
                for j in range(3):
                    if (board[i][j] == ttt.EMPTY and tiles[i][j].collidepoint(mouse)):
                        if (game_mode == "2P") or (game_mode == "AI" and user == player):
                            board = ttt.result(board, (i, j))
                        # elif game_mode == "AI" and user == player:
                        #     board = ttt.result(board, (i, j))

        # game over terminal - Play again button
        if game_over:
    ...
    improvement:
        ...
        Home screen : AI mode - 2 players
            2players: start - score - choose player1 / player2 : pick from list or make a new one   

        code_layout: separate class, categorize ...

        for 2 players mode: make screen to input names and choose name, ranking
                        implementation score
                        after game over: continue (between 2 players - update score), home
    """